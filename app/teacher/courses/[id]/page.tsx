...
export default async function TeacherCourseNotebookPage(props: { params: Promise<{ id: string }>, searchParams: Promise<{ action?: string }> }) {
    ...
    try {
        ...
        const [courseData, evalsData] = await Promise.all([
            apiFetch<any>(`/courses/${courseId}`),
            apiFetch<any[]>(`/grades/evaluations/course/${courseId}`)
        ]);
        course = courseData;
        evaluations = evalsData;
    } catch (error: any) {
        return (
            <div className="p-8 font-black text-red-500 uppercase tracking-widest text-sm text-center border-4 border-red-500 bg-red-50">
                Error de API: {error.message}. Curso no encontrado.
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 relative">
            ...
            <CreateEvaluationModal
                ...
                returnPath={`/teacher/courses/${courseId}`}
            />
                ...
                {evaluations.length === 0 ? (
                    ...
                    evaluations.map((evaluation: any) => {
                        ...
                        return (
                            <Link
                                key={evaluation.id}
                                href={`/teacher/courses/${courseId}/evaluate/${evaluation.id}`}
                            ...
                            >
                                ...
                                <div className="mt-4 md:mt-0 flex items-center gap-6 self-stretch md:self-auto">
                                    <div className="text-right hidden sm:block">
                                        ...
                                        <span className="block text-sm font-black text-uecg-black">{evaluation._count?.marks || 0} Alumnos</span>
                                    </div>
