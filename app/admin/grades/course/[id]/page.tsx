...
export default async function CourseNotebookPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ action?: string }> }) {
    ...
    if (!course) return <div className="p-8 font-black text-red-500">Curso no encontrado.</div>;

    // Obtener las evaluaciones de este curso ordenadas por fecha
            ...
            <div className="grid grid-cols-1 gap-4">
                {evaluations.length === 0 ? (
                    <div className="p-16 text-center border-4 border-dashed border-gray-200 bg-gray-50">
                        ...
                        <span className="text-xs font-bold text-gray-400 uppercase">Aún no has registrado notas para esta materia.</span>
                    </div>
                ) : (
                        ...
                        >
                            ...
                            <div className="mt-4 md:mt-0 flex items-center gap-6">
                                <div className="text-right">
                                    <span className="block text-[10px] font-black uppercase tracking-widest text-uecg-gray mb-1">Progreso de Calificación</span>
                                    <span className="block text-sm font-black text-uecg-blue">{evaluation._count.marks} Alumnos calificados</span>
                                </div>
