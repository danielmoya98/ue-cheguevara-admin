...
export default async function GradesPortalPage() {
    // En el futuro, aquí pasarías el ID del profesor logueado.
    // Por ahora, al no pasar ID, traerá TODAS las materias para que el Admin las vea.
    const { data: courses, success } = await getTeacherCoursesAction();

    if (!success || !courses) {
        return <div className="p-8 text-red-500 font-black">Error al cargar las asignaturas.</div>;
    }
        ...
        <div className="space-y-8 relative">
            ...
            <div className="border-b-4 border-uecg-black pb-6">
                ...
                <p className="text-uecg-gray font-bold text-xs uppercase tracking-widest mt-2">
                    Seleccione una asignatura para gestionar el cuaderno pedagógico.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {courses.length === 0 && (
                    <div className="col-span-full p-12 text-center border-4 border-dashed border-gray-200">
                        <span className="font-black text-uecg-gray uppercase tracking-widest text-sm">No tiene materias asignadas este año</span>
                    </div>
                )}
                ...
                {courses.map((course) => (
                    <Link
                        ...
                        className="group flex flex-col bg-white border-2 border-uecg-line hover:border-uecg-blue transition-colors relative overflow-hidden h-40"
                    >
